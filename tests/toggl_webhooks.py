...
from togglsync.togglws import TogglClient, TogglSocketMessage
...
def generate_message_handler(method: str, url: str, **kwargs):
    async def message_handler_outer(action, model, msg: TogglSocketMessage):
        await message_handler_nopost(action, model, method, url, msg, **kwargs)
    ...
    **request_kwargs,
    ...
    while True:
        ...
        try:
            ...
            logger.warning(
                f"{action} {model} -> {method} {url} failed, trial: {trial}. retrying"
            )
            logger.exception(e)
            if trial >= trials:
        ...
        if res.status_code >= 400:
            logger.warning(
                f"{action} {model} -> {res.status_code} {method} {url} failed, trial: {trial}. retrying"
            )
            logger.debug(
                f"{action} {model} -> {res.status_code} {method} {url} response: {res.text}"
            )
            if trial >= trials:
                logger.error(f"{action} {model} -> {method} {url} stop trials")
                break
...
async def message_handler_nopost(
    action: str,
    model: str,
    method: str,
    url: str,
    ...
    **kwargs,
    ...
    logger.info(f"{msg} {action} {model} ->  {method} {url} response: None")
